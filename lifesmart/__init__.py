...
def lifesmart_Sendkeys(appkey, apptoken, usertoken, userid, agt, ai, me, category, brand, keys):
    ...
    tick = int(time.time())
    # keys = str(keys)
    sdata = "method:SendKeys,agt:" + agt + ",ai:" + ai + ",brand:" + brand + ",category:" + category + ",keys:" + keys + ",me:" + me + ",time:" + str(
        tick) + ",userid:" + userid + ",usertoken:" + usertoken + ",appkey:" + appkey + ",apptoken:" + apptoken
    sign = hashlib.md5(sdata.encode(encoding='UTF-8')).hexdigest()
...
def lifesmart_Sendackeys(appkey, apptoken, usertoken, userid, agt, ai, me, category, brand, keys, power, mode, temp,
                         wind, swing):
...
def setup(hass, config):
    ...
    def send_keys(call):
        ...
        keys = call.data['keys']
        restkey = lifesmart_Sendkeys(param['appkey'], param['apptoken'], param['usertoken'], param['userid'], agt, ai,
                                     me, category, brand, keys)
    ...
    def send_ackeys(call):
        ...
        restackey = lifesmart_Sendackeys(param['appkey'], param['apptoken'], param['usertoken'], param['userid'], agt,
                                         ai, me, category, brand, keys, power, mode, temp, wind, swing)
    ...
    def on_close(ws):
        _LOGGER.warning("lifesmart websocket closed...")
    ...
    def on_open(ws):
        ...
        send_data = json.dumps(send_values)
        ws.send(send_data)
        _LOGGER.warning("lifesmart websocket sending_data...")

    hass.services.register(DOMAIN, 'send_keys', send_keys)
    hass.services.register(DOMAIN, 'send_ackeys', send_ackeys)
    ws = websocket.WebSocketApp("wss://api.cn2.ilifesmart.com:8443/wsapp/",
...
class LifeSmartStatesManager(threading.Thread):
    ...
    def run(self):
        while self._run:
            _LOGGER.info('lifesmart: starting wss...')
            self._ws.run_forever()
            _LOGGER.info('lifesmart: restart wss...')
            time.sleep(10)
